...
export function Intersecter(selfIntersection, eps, buildLog) {
  ...
  const segmentCopy = (start, end, { myFill: { above, below } }) => ({
    id: buildLog ? buildLog.segmentId() : -1,
    start,
    end,
    myFill: { above, below },
  ...
  const event_root = LinkedList.create();

  const eventCompare = (p1_isStart, p1_1, p1_2, p2_isStart, p2_1, p2_2) => {
  ...
  const eventAddSegmentStart = (seg, primary) => {
  ...
  const eventAddSegmentEnd = (ev_start, seg, primary) => {
  ...
  const eventAddSegment = (seg, primary) => {
    var ev_start = eventAddSegmentStart(seg, primary);
    eventAddSegmentEnd(ev_start, seg, primary);
    return ev_start;
  };

  const eventUpdateEnd = (ev, end) => {
  ...
  const eventDivide = (ev, pt) => {
    const ns = segmentCopy(pt, ev.seg.end, ev.seg);
    eventUpdateEnd(ev, pt);
    return eventAddSegment(ns, ev.primary);
  };

  const calculate = (primaryPolyInverted, secondaryPolyInverted) => {
    ...
    function checkBothIntersections(above, below, ev) {
      if (above) {
    ...
    while (!event_root.isEmpty()) {
      ...
      if (ev.isStart) {
        ...
        var eve = checkBothIntersections(above, below, ev);
        if (eve) {
